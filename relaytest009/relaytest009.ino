...
//--------------Relay Control set variables
#define relaypin1 32
#define relaypin2 34
#define relaypin3 36
#define relaypin4 38

//--------------Keypads variables set
#define lcdRS 8
#define lcdEnable 9
#define lcdD4 4
#define lcdD5 5
...
//--------------IR Control set variables
#define RECV_PIN 11
IRrecv irrecv(RECV_PIN);
decode_results results;
long IR_key = -1;
//--------------counter set variables
...
void loop()
{
   int Temperatureresult = DHT11.acquireAndWait();
    if (Temperatureresult==IDDHTLIB_OK);
      {
           showTemperature();
      }
  ...
  // following code is for controling set time functions
  if ((IR_key == 11) and (setdelay == 0))
  {
    setdelaytime = 0 ;
    setdelay = 1  ;
  }
  while(setdelay == 1)
  {
     ...
     if (IR_key == 11)
     {
       ...
       if (IR_key == 20)
        ...
        if (setdelaytime > 0 )
          {
            counter = 1 ;
            countdelaytime = setdelaytime ;
            setdelay = 0  ;
             setcountingtime = RTC.now() ;
            showcounttime() ;
            break ;
          }
          else
    ...
    showsettime() ;
    Serial.print("IR Code( in side while)  is :");
  Serial.println(IR_key) ;
  if (counter == 1 )
    {
     counting(); 
   ...
   delay(500);
  }      // above code is for controling set time functions
  ...
  if ((lcd_key == btnLEFT) or (IR_key == 1))
  {
    // speed = 1
      LCDClear();
      lcd.setCursor(11,1);
      lcd.print("One"); // print a simple message
  ...
  if ((lcd_key == btnUP) or (IR_key == 2))
  {
    // speed = 2
      LCDClear();
      lcd.setCursor(11,1);
      lcd.print("Two"); // print a simple message
       digitalWrite(relaypin1,HIGH);
  ...
  if ((lcd_key == btnDOWN) or (IR_key == 3))
  {
    // speed = 3
      LCDClear();
      lcd.setCursor(11,1);
      lcd.print("Three"); // print a simple message
       digitalWrite(relaypin1,HIGH);
       digitalWrite(relaypin2, HIGH);
  ...
  if ((lcd_key == btnRIGHT) or (IR_key == 4))
  {
    // speed = 4
      LCDClear();
      lcd.setCursor(11,1);
      lcd.print("Four"); // print a simple message
       digitalWrite(relaypin1,HIGH);
       digitalWrite(relaypin2, HIGH);
      digitalWrite(relaypin3, HIGH);
  ...
  if ((lcd_key == btnSELECT) or (IR_key == 0))
  {
      ...
      LCDClear();
      lcd.setCursor(11,1);
        lcd.print("Stop"); // print a simple message
       digitalWrite(relaypin1,HIGH);
       digitalWrite(relaypin2, HIGH);
      digitalWrite(relaypin3, HIGH);
      digitalWrite(relaypin4, HIGH);
...
void counting()
{
  // this function is to counting
   DateTime tmpnow = RTC.now();
  countdelaytime = ((tmpnow.day() - setcountingtime.day()) *24 +(tmpnow.hour() - setcountingtime.hour()) ) * 60 + (tmpnow.minute() - setcountingtime.minute()) ;
  showcounttime() ;
  if (countdelaytime <= 0)
  {
    counter = 0 ;
...
void showsettime()
{
  // this function is to show adjust counting time in  LCD 
  lcd.setCursor(10,0);
  lcd.print("@"); // print a simple message
  lcd.setCursor(11,0);
  lcd.print(setdelaytime); // print a simple message
}  

void clearsettime()
{
  // this function is to clear LCD for counting time
  lcd.setCursor(10,0);
  lcd.print("        "); // print a simple message
}  
...
void showcounttime()
{
  // this function is show counter time in LCD
  lcd.setCursor(10,0);
  lcd.print("#"); // print a simple message
  lcd.setCursor(11,0);
  lcd.print(countdelaytime); // print a simple message
}  

//-----DHT11 user functions ----
void dht11_wrapper() {
  DHT11.isrCallback();
}
...
{
  // this function show data for Temperature & Humidity Sensor 
  lcd.setCursor(9,1);
  lcd.print(DHT11.getCelsius()); // print a simple message
//  lcd.setCursor(15,1);
